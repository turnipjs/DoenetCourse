...
$mergeFirstName = array_map(
    function($branchId) use($conn) {
        return mysqli_real_escape_string($conn, $branchId);
    },
    $_POST['mergeFirstName']
);
$mergeLastName = array_map(
    function($branchId) use($conn) {
        return mysqli_real_escape_string($conn, $branchId);
    },
    $_POST['mergeLastName']
);
$mergeEmail = array_map(
    function($branchId) use($conn) {
        return mysqli_real_escape_string($conn, $branchId);
    },
    $_POST['mergeEmail']
);
$mergeSection = array_map(
    function($branchId) use($conn) {
        return mysqli_real_escape_string($conn, $branchId);
    },
    $_POST['mergeSection']
);
$mergeDropped = array_map(
    function($branchId) use($conn) {
        return mysqli_real_escape_string($conn, $branchId);
    },
    $_POST['mergeDropped']
...
for($i = 0; $i < $dataLength; $i++){
    ...
    if (in_array("dropped",$mergeHeads,false) && $mergeDropped[$i] == "Dropped"	) {
        $withdrew = "1";
    }
    ...
    if (($id != "0" && in_array($id,$db_empids,FALSE)) || in_array($email,$db_emails,FALSE)){
        ...
        $sql = "INSERT INTO course_enrollment
                    (courseId,userId,firstName,lastName,email,empId,dateEnrolled,section)
                VALUES ('$courseId','$userId','$firstName','$lastName','$email','$id',NOW(),'$section')
        ";
        $result = $conn->query($sql);
    }
}
